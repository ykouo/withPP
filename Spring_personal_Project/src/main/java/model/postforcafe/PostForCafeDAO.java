...
class PostForCafeRowMapper implements RowMapper<PostForCafeVO> {
	...
	public PostForCafeVO mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		vo.setLocaddress(rs.getString("locaddress"));
		vo.setLoccall(rs.getString("loccall"));
...
public class PostForCafeDAO {
	...
	// SQL문
	private final String insertPostSQL = "INSERT INTO POST_FOR_CAFE (PNUM,MID,NICKNAME,TITLE,THUMNAIL,CONTENT,LOCADDRESS,LOCCALL,LOCATION) VALUES ((SELECT NVL(MAX(PNUM),0)+1 FROM POST_FOR_CAFE),?,?,?,?,?,?,?,?)";
	private final String insertPostSQLNothumnail = "INSERT INTO POST_FOR_CAFE (PNUM,MID,NICKNAME,TITLE,THUMNAIL,CONTENT,LOCADDRESS,LOCCALL,LOCATION) VALUES ((SELECT NVL(MAX(PNUM),0)+1 FROM POST_FOR_CAFE),?,?,?,'images/thumnail.png',?,?,?,?)";
	private final String updatePostSQL = "UPDATE POST_FOR_CAFE SET NICKNAME=?,TITLE=?,THUMNAIL=?,CONTENT=?,LOCADDRESS=?,LOCCALL=?,LOCATION=? WHERE PNUM=?";
	...
	public void insertPost(PostForCafeVO vo) {
		...
		Object[] args = {vo.getMid(),vo.getNickname(),vo.getTitle(),vo.getThumnail(),vo.getContent(),vo.getLocaddress(),vo.getLoccall(),vo.getLocation()};
		jdbcTemplate.update(insertPostSQL,args);
	}
	public void insertPostNoTumnail(PostForCafeVO vo) {
		System.err.println("PostForCafe DAO 출력 : insertPost ");
		System.out.println("PostForCafe DAO 출력 : insertPost " + vo);
		Object[] args = {vo.getMid(),vo.getNickname(),vo.getTitle(),vo.getContent(),vo.getLocaddress(),vo.getLoccall(),vo.getLocation()};
	...
	public void updatePost(PostForCafeVO vo) {
		...
		Object[] args = {vo.getNickname(),vo.getTitle(),vo.getThumnail(),vo.getContent(),vo.getLocaddress(),vo.getLoccall(),vo.getLocation(),vo.getPnum()};
...
/*	public List<PostForCafeVO> searchPost(PostForCafeVO vo) {
		System.out.println("PostForCafe DAO 출력 : searchPost");
...
// =================================================================================
	...
	public List<PostForCafeVO> getPostListPagenation(int startRow, int endRow) {
		...
		String getPostPagenationSQL = "SELECT * FROM (SELECT ROWNUM AS RNUM, A.PNUM,A.MID,A.NICKNAME,A.TITLE,A.THUMNAIL,A.CONTENT,A.LOCADDRESS,A.LOCCALL,A.LOCATION,A.CNT,A.HEART,A.WDATE FROM (SELECT * FROM POST_FOR_CAFE ORDER BY PNUM DESC) A) WHERE RNUM BETWEEN "+ startRow + " AND " + endRow;
