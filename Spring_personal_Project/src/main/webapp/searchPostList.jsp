...
<html lang="en">
  ...
  <body data-spy="scroll" data-target=".site-navbar-target" data-offset="300">
            ...
            <div class="container">
              <div class="row align-items-center justify-content-center">
                <div class="col-lg-6 text-center">
                  <h1 style="color:gold;">SEARCH POST LIST</h1> 
		...
		<div class="row" style="justify-content: center;">
			<ul id="paging" class="pagenation">
					<c:if test="${paging.pageNum==paging.firstPageNum}">
						 <span class="p-3"><a href="searchPost.do?page=${paging.prevPageNum}&condition=${condition}&keyword=${keyword}">prev</a></span>
					</c:if>
					<c:if test="${paging.pageNum!=paging.firstPageNum}">
						<span class="p-3"><a
							href="searchPost.do?page=${paging.prevPageNum}&condition=${condition}&keyword=${keyword}">prev</a></span>
					</c:if>
					<c:forEach var="i" begin="${paging.startPageNum}"
						...
						<c:choose>
							...
							<c:otherwise>
								<span class="p-3">
									<a href="searchPost.do?page=${i}&condition=${condition}&keyword=${keyword}"class="page">${i}</a>
								</span>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					...
					<c:if test="${paging.pageNum==paging.finalPageNum}">
						<span class="p-3">
							<a href="searchPost.do?page=${paging.nextPageNum}&condition=${condition}&keyword=${keyword}"class="button disabled">next</a>
						</span>
					</c:if>
					<c:if test="${paging.pageNum!=paging.finalPageNum}">
						<span class="p-3">
							<a href="searchPost.do?page=${paging.nextPageNum}&condition=${condition}&keyword=${keyword}"class="button">next</a>
						</span>
					</c:if>
			</ul>
		</div>
    ...
    <div class="container">
	<div class="row" style="justify-content: center;">
			<a href="showPostList.do">게시글 목록으로 </a>
	</div>
	</div>
